...
data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/Animated Line Charts/Jose Ramirez Comp.csv")
...
cols <- c("Bobby Murcer" = "#0C2340", "Hank Blalock" = "#C0111F", "Joe Gordon" = "#C4CED3",
          "Richie Hebner" = "#FDB827", "Shawn Green" = "#005A9C", "Jose Ramirez" = "#E31937")
...
p <- data %>%
  ...
  scale_x_continuous(breaks = seq(19, 37, 1)) +
  ...
  ease_aes("linear") +
  #view_follow() +
  labs(title = "Jose Ramirez WAR vs. Top 5 Bill James Similarity Scores", subtitle = "Only Includes Players Who Have Finished Their Career", x = "Season Age", y = "Career WAR")

#Animate the visual

anim <- animate(p, nframes = 24, fps = 1, width = 800, height = 482, end_pause = 5, renderer = magick_renderer())

anim_save("Jose Ramirez Comp.gif", anim)