...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/P")
...
pitcher_data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/P/P WAR Leaders Bar Chart Race.csv")
...
pitcher_data$Era <- factor(pitcher_data$Era, levels = c("19th Century", "Dead Ball", "Live Ball", "Integration", "Expansion", "Free Agency", "Steroid", "Modern"))

pitcher_data_wide <- pitcher_data %>%
  spread(Season, WAR)

#Replace null values with zeros to create WAR totals for years where player is no longer active

pitcher_data_wide[is.na(pitcher_data_wide)] <- 0.0

#Go back to long data format

pitcher_data <- pitcher_data_wide %>%
  gather(Season, WAR, "1871":"2020")

#Change Year column to integer

pitcher_data$Season <- as.integer(pitcher_data$Season)
...
p <- top_data %>%
  ...
  labs(title = "Top 10 Pitchers by Career WAR",
...
anim_save("P_WAR_Bar_Race.gif", anim)