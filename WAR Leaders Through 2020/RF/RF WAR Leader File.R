...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/RF")
...
right_field_data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/RF/RF WAR Leaders Bar Chart Race.csv")

#Make data wide so that I can replace null values with zeros

right_field_data <- right_field_data %>%
...
right_field_data$Era <- factor(right_field_data$Era, levels = c("19th Century", "Dead Ball", "Live Ball", "Integration", "Expansion", "Free Agency", "Steroid", "Modern"))

right_field_data_wide <- right_field_data %>%
  spread(Season, WAR)

#Replace null values with zeros to create WAR totals for years where player is no longer active

right_field_data_wide[is.na(right_field_data_wide)] <- 0.0

#Go back to long data format

right_field_data <- right_field_data_wide %>%
  gather(Season, WAR, "1871":"2020")

#Change Year column to integer

right_field_data$Season <- as.integer(right_field_data$Season)
...
p <- top_data %>%
  ...
  labs(title = "Top 10 Right Fielders by Career WAR",
...
anim_save("RF_WAR_Bar_Race.gif", anim)