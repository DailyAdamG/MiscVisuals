...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/3B")
...
thirdbase_data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/3B/3B WAR Leaders Bar Chart Race.csv")

#Make data wide so that I can replace null values with zeros

thirdbase_data <- thirdbase_data %>%
...
thirdbase_data$Era <- factor(thirdbase_data$Era, levels = c("19th Century", "Dead Ball", "Live Ball", "Integration", "Expansion", "Free Agency", "Steroid", "Modern"))

thirdbase_data_wide <- thirdbase_data %>%
  spread(Season, WAR)

#Replace null values with zeros to create WAR totals for years where player is no longer active

thirdbase_data_wide[is.na(thirdbase_data_wide)] <- 0.0

#Go back to long data format

thirdbase_data <- thirdbase_data_wide %>%
  gather(Season, WAR, "1871":"2020")

#Change Year column to integer

thirdbase_data$Season <- as.integer(thirdbase_data$Season)
...
p <- top_data %>%
  ...
  labs(title = "Top 10 Third Basemen by Career WAR",
...
anim_save("3B_WAR_Bar_Race.gif", anim)