...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/CF")
...
center_field_data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/WAR Leaders Through 2020/CF/CF WAR Leaders Bar Chart Race.csv")

#Make data wide so that I can replace null values with zeros

center_field_data <- center_field_data %>%
...
center_field_data$Era <- factor(center_field_data$Era, levels = c("19th Century", "Dead Ball", "Live Ball", "Integration", "Expansion", "Free Agency", "Steroid", "Modern"))

center_field_data_wide <- center_field_data %>%
  spread(Season, WAR)

#Replace null values with zeros to create WAR totals for years where player is no longer active

center_field_data_wide[is.na(center_field_data_wide)] <- 0.0

#Go back to long data format

center_field_data <- center_field_data_wide %>%
  gather(Season, WAR, "1871":"2020")

#Change Year column to integer

center_field_data$Season <- as.integer(center_field_data$Season)
...
p <- top_data %>%
  ...
  labs(title = "Top 10 Center Fielders by Career WAR",
...
anim_save("CF_WAR_Bar_Race.gif", anim)