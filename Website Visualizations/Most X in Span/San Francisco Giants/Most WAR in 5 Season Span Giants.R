...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/San Francisco Giants")
...
data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/San Francisco Giants/Most WAR in 5 Season Span Giants.csv")
...
no_dupes <- no_dupes %>%
  filter(Name != 'Barry Bonds' | MIN != 1994 & MIN != 1995) %>%
  filter(Name != 'Willie Mays' | MIN != 1955 & MIN != 1956 & MIN != 1957)
...
top_data %>%
  ...
  coord_flip(ylim = c(30, 55)) +
  scale_y_continuous(breaks = seq(30, 55, 5)) +
  scale_fill_manual(values = c("Odd" = "#FD5A1E", "Even" = "#27251F")) +
  ...
  labs(title = "San Francisco Giants Franchise History\nMost WAR in a 5 Season Span",
