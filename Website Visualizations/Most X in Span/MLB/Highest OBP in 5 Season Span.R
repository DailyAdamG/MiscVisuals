...
no_dupes <- no_dupes %>%
  filter(nameLast != 'Ruth' | MIN != 1930 & MIN != 1928)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, OBP), y = OBP, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(OBP,3)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(.460, .540)) +
  scale_y_continuous(breaks = seq(.460, .540, .005), labels = numformat) +
  ...
  labs(title = "Highest OBP in a 5 Season Span\n(Minimum of 2500 Plate Appearances)",
