...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, GROUP_CONCAT(TeamsFranchises.franchID) AS FranchIDS, SUM(ifnull(Pitching.ER,0)) AS ER, SUM(ifnull(Pitching.IPOuts,0)) AS Outs
                    FROM People
                    INNER JOIN Pitching
...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(ER_Total = rollapply(ER, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
  mutate(Outs_Total = rollapply(Outs, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
...
display_data <- sqldf('SELECT firstYear AS MIN,
      ...
      ER_Total,
      Outs_Total,
      ((ER_Total * 27.00000) / Outs_Total) AS ERA,
      OneFranchise
      FROM data
      WHERE Outs_Total >= 2250
      AND Range >= 4
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, -ERA), y = ERA, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(ERA,2)), fontface = "bold", hjust = -0.3) +
  ...
  labs(title = "Lowest ERA in a 5 Season Span in MLB History\n(Minimum of 750 IP)",
