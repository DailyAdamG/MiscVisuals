...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.RBI) AS RBI
                    FROM People
                    INNER JOIN Batting
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, RBI_Total), y = RBI_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = RBI_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(650, 825)) +
  scale_y_continuous(breaks = seq(650, 825, 25)) +
  ...
  labs(title = "Most RBI in a 5 Season Span",
