...
top_data %>%
  ggplot(aes(x = reorder(chart_label, SO_Total), y = SO_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = SO_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(900, 1100)) +
  scale_y_continuous(breaks = seq(900, 1100, 20)) +
  ...
  labs(title = "Most Strikeouts in a 3 Season Span",
