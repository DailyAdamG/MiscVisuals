...
top_data %>%
  ggplot(aes(x = reorder(chart_label, BA), y = BA, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(BA,3)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(.375, .405)) +
  scale_y_continuous(breaks = seq(.375, .405, .005), labels = numformat) +
  ...
  labs(title = "Highest Batting Average in a 5 Season Span\n(Minimum of 2500 Plate Appearances)",
