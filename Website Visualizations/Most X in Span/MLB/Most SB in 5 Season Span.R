...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.SB) AS SB
                    FROM People
                    INNER JOIN Batting
...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(SB_Total = rollapply(SB, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data <- no_dupes %>%
  mutate(Rank = rank(-SB_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, SB_Total), y = SB_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = SB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(375, 475)) +
  scale_y_continuous(breaks = seq(375, 475, 10)) +
  ...
  labs(title = "Most Stolen Bases in a 5 Season Span",
