...
top_data <- top_data %>%
  mutate(DisplayFranchises = ifelse(OneFranchise == "No", "Multiple Franchises", Teams))
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, SLG), y = SLG, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(SLG,3)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(.655, .785)) +
  scale_y_continuous(breaks = seq(.655, .785, .005), labels = numformat) +
  ...
  labs(title = "Highest SLG in a 5 Season Span\n(Minimum of 2500 Plate Appearances)",
