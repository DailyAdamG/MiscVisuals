...
data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/MLB/Most Pitching WAR in 3 Season Span.csv")

data <- filter(data, Season >= 1901)
...
data <- data %>%
  ...
  mutate(prevTeam = lag(Team,2)) %>%
  mutate(prevTeam = ifelse(is.na(prevTeam), Team, prevTeam)) %>%
  mutate(prevTeam_2 = lag(Team,1)) %>%
  mutate(prevTeam_2 = ifelse(is.na(prevTeam_2), Team, prevTeam_2)) %>%
  mutate(OneFranchise = ifelse(Team == prevTeam & Team == prevTeam_2, "Yes", "No")) %>%
  ungroup()
...
top_data %>%
  ...
  labs(title = "Most Pitching WAR in a 3 Season Span (Modern Era)",
