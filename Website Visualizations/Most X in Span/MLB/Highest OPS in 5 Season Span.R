...
top_data %>%
  ggplot(aes(x = reorder(chart_label, OPS), y = OPS, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(OPS,3)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(1.100, 1.340)) +
  scale_y_continuous(breaks = seq(1.100, 1.340, .020), labels = numformat) +
  ...
  labs(title = "Highest OPS in a 5 Season Span\n(Minimum of 2500 Plate Appearances)",
