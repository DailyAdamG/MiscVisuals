...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.BB) AS BB
                    FROM People
                    INNER JOIN Batting
...
no_dupes <- no_dupes %>%
  filter(nameLast != 'Bonds' | (MIN != 1995 & MIN != 1993)) %>%
  filter(nameLast != 'Ruth' | MIN != 1930)
...
top_data <- no_dupes %>%
  mutate(Rank = rank(-BB_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, BB_Total), y = BB_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = BB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(600, 900)) +
  ...
  labs(title = "Most Walks in a 5 Season Span",
