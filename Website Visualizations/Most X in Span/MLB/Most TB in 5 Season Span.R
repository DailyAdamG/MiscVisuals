...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.H) + SUM(Batting.X2B) + SUM(Batting.X3B) * 2 + SUM(Batting.HR) * 3 AS TB
                    FROM People
                    INNER JOIN Batting
...
top_data <- no_dupes %>%
  mutate(Rank = rank(-TB_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, TB_Total), y = TB_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = TB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(1800, 2000)) +
  scale_y_continuous(breaks = seq(1800, 2000, 20)) +
  ...
  labs(title = "Most Total Bases in a 5 Season Span",
