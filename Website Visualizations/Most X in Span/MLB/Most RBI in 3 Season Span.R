...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(RBI_Total = rollapply(RBI, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(400, 510)) +
  scale_y_continuous(breaks = seq(400, 510, 10)) +
  ...
  labs(title = "Most RBI in a 3 Season Span",
