...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.X2B) AS Doubles
                    FROM People
                    INNER JOIN Batting
...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(Doubles_Total = rollapply(Doubles, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
  ...
  mutate(prevTeam = lag(Teams,4)) %>%
  mutate(prevTeam = ifelse(is.na(prevTeam), Teams, prevTeam)) %>%
  mutate(prevTeam_2 = lag(Teams,3)) %>%
  mutate(prevTeam_2 = ifelse(is.na(prevTeam_2), Teams, prevTeam_2)) %>%
  mutate(prevTeam_3 = lag(Teams,2)) %>%
  mutate(prevTeam_3 = ifelse(is.na(prevTeam_3), Teams, prevTeam_3)) %>%
  mutate(prevTeam_4 = lag(Teams,1)) %>%
  mutate(prevTeam_4 = ifelse(is.na(prevTeam_4), Teams, prevTeam_4)) %>%
  mutate(OneFranchise = ifelse(Teams == prevTeam & Teams == prevTeam_2 & Teams == prevTeam_3 & Teams == prevTeam_4, "Yes", "No")) %>%
  ungroup()
...
no_dupes <- display_data %>%
  ...
  slice_head() %>%
  arrange(-Doubles_Total) %>%
  ungroup()

#Limit to top 10 spots

top_data <- no_dupes %>%
  mutate(Rank = rank(-Doubles_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, Doubles_Total), y = Doubles_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Doubles_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(200, 270)) +
  scale_y_continuous(breaks = seq(200, 270, 10)) +
  ...
  labs(title = "Most Doubles in a 5 Season Span",
