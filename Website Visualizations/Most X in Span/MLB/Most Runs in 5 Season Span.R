...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.R) AS R
                    FROM People
                    INNER JOIN Batting
...
no_dupes <- no_dupes %>%
  filter(nameLast != 'Gehrig' | MIN != 1933)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, R_Total), y = R_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = R_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(650, 800)) +
  scale_y_continuous(breaks = seq(650, 800, 10)) +
  ...
  labs(title = "Most Runs in a 5 Season Span",
