...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Batting.X3B) AS Triples
                    FROM People
                    INNER JOIN Batting
...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(Triples_Total = rollapply(Triples, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
...
no_dupes <- display_data %>%
  ...
  slice_head() %>%
  arrange(-Triples_Total) %>%
  ungroup()

#Limit to top 10 spots

top_data <- no_dupes %>%
  mutate(Rank = rank(-Triples_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, Triples_Total), y = Triples_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Triples_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(90, 105)) +
  scale_y_continuous(breaks = seq(90, 105, 1)) +
  ...
  labs(title = "Most Triples in a 5 Season Span",
