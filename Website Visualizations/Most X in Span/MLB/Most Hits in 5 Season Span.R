...
top_data <- no_dupes %>%
  mutate(Rank = rank(-H_Total, ties.method = "min")) %>%
  filter(Rank <= 10)
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, H_Total), y = H_Total, fill = DisplayFranchises)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = H_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(1050, 1150)) +
  scale_y_continuous(breaks = seq(1050, 1150, 10)) +
  ...
  labs(title = "Most Hits in a 5 Season Span",
