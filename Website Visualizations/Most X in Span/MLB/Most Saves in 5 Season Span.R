...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(SV_Total = rollapply(SV, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(200, 230)) +
  scale_y_continuous(breaks = seq(200, 230, 5)) +
  ...
  labs(title = "Most Saves in a 5 Season Span",
