...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/New York Yankees")
...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.SB) AS SB
                    FROM People
                    INNER JOIN Batting
                    ...
                    WHERE TeamsFranchises.franchName = "New York Yankees"
...
top_data <- no_dupes %>%
  arrange(-SB_Total, desc(nameFirst)) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, SB_Total), y = SB_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = SB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(130, 330)) +
  scale_y_continuous(breaks = seq(130, 330, 10)) +
  scale_fill_manual(values = c("Odd" = "#0C2340", "Even" = "#C4CED3")) +
  ...
  labs(title = "New York Yankees Franchise History\nMost Stolen Bases in a 5 Season Span",
