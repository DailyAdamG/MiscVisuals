...
data <- read.csv("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/New York Yankees/Most WAR in 5 Season Span Yankees.csv")
...
no_dupes <- no_dupes %>%
  filter(Name != 'Babe Ruth' | MIN != 1929) %>%
  filter(Name != 'Mickey Mantle' | MIN != 1960)
...
top_data %>%
  ...
  coord_flip(ylim = c(20, 36)) +
  scale_y_continuous(breaks = seq(20, 36, 1)) +
