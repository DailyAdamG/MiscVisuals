...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(SB_Total = rollapply(SB, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(100, 225)) +
  scale_y_continuous(breaks = seq(100, 225, 5)) +
