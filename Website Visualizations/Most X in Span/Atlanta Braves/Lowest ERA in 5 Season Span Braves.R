...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID AS FranchIDS, SUM(ifnull(Pitching.ER,0)) AS ER, SUM(ifnull(Pitching.IPOuts,0)) AS Outs
                    FROM People
                    INNER JOIN Pitching
                    ...
                    WHERE TeamsFranchises.franchName = "Atlanta Braves"
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, -ERA), y = ERA, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(ERA,2)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(2.05, 2.75)) +
  scale_y_continuous(breaks = seq(2.05, 2.75, 0.05)) +
  scale_fill_manual(values = c("Odd" = "#CE1141", "Even" = "#13274F")) +
  ...
  labs(title = "Atlanta Braves Franchise History\nLowest ERA in a 5 Season Span (Minimum of 750 IP)",
