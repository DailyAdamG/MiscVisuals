...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(ER_Total = rollapply(ER, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
  mutate(Outs_Total = rollapply(Outs, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(1.85, 2.50)) +
  scale_y_continuous(breaks = seq(1.85, 2.50, 0.05)) +
  ...
  labs(title = "Atlanta Braves Franchise History\nLowest ERA in a 3 Season Span (Minimum of 450 IP)",
