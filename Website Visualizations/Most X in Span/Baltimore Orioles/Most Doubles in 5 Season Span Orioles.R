...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/Baltimore Orioles")
...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.X2B) AS Doubles
                    FROM People
                    INNER JOIN Batting
                    ...
                    WHERE TeamsFranchises.franchName = "Baltimore Orioles"
...
top_data <- no_dupes %>%
  arrange(-Doubles_Total, desc(nameFirst)) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, Doubles_Total), y = Doubles_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Doubles_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(160, 230)) +
  scale_y_continuous(breaks = seq(160, 230, 5)) +
  scale_fill_manual(values = c("Odd" = "#DF4601", "Even" = "#000000")) +
