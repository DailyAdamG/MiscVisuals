...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(R_Total = rollapply(R, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(340, 480)) +
  scale_y_continuous(breaks = seq(340, 480, 10)) +
