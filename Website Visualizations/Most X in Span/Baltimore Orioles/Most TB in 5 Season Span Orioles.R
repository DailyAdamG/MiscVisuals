...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.H) + SUM(Batting.X2B) + SUM(Batting.X3B) * 2 + SUM(Batting.HR) * 3 AS TB
                    FROM People
                    INNER JOIN Batting
...
top_data <- no_dupes %>%
  arrange(-TB_Total, desc(nameFirst)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(1380, 1660)) +
  scale_y_continuous(breaks = seq(1380, 1660, 20)) +
