...
top_data %>%
  ggplot(aes(x = reorder(chart_label, SLG), y = SLG, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = rd(SLG,3)), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(.535, .605)) +
  scale_y_continuous(breaks = seq(.535, .605, .005), labels = numformat) +
