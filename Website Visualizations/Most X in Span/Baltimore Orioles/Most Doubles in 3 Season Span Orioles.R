...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(Doubles_Total = rollapply(Doubles, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(100, 150)) +
  scale_y_continuous(breaks = seq(100, 150, 2)) +
