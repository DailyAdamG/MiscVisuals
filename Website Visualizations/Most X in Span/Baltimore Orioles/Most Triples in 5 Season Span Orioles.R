...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.X3B) AS Triples
                    FROM People
                    INNER JOIN Batting
...
top_data <- no_dupes %>%
  arrange(-Triples_Total, desc(nameFirst)) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, Triples_Total), y = Triples_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = Triples_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(45, 85)) +
  scale_y_continuous(breaks = seq(45, 85, 5)) +
