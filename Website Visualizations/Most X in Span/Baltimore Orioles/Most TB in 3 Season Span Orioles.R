...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(TB_Total = rollapply(TB, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data <- no_dupes %>%
  arrange(-TB_Total, desc(nameFirst), -MIN) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, TB_Total), y = TB_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = TB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(900, 1080)) +
  scale_y_continuous(breaks = seq(900, 1080, 10)) +
