...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.BB) AS BB
                    FROM People
                    INNER JOIN Batting
...
top_data <- no_dupes %>%
  arrange(-BB_Total, desc(nameFirst)) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, BB_Total), y = BB_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = BB_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(400, 560)) +
  scale_y_continuous(breaks = seq(400, 560, 10)) +
