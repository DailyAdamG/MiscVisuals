...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(Triples_Total = rollapply(Triples, width = 3, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(35, 60)) +
  scale_y_continuous(breaks = seq(35, 60, 1)) +
