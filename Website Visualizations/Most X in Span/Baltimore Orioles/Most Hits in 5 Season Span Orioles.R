...
top_data <- no_dupes %>%
  arrange(-H_Total, desc(nameFirst)) %>%
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, H_Total), y = H_Total, fill = EvenOdd)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = H_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(870, 1150)) +
  scale_y_continuous(breaks = seq(870, 1150, 20)) +
