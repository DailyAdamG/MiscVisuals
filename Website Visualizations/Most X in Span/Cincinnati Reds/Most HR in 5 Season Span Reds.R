...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/Cincinnati Reds")
...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.HR) AS HR
                    ...
                    WHERE TeamsFranchises.franchName = "Cincinnati Reds"
...
no_dupes <- no_dupes %>%
  filter(nameFirst != 'David' & nameLast != 'Ortiz' | MIN != 2010)
...
top_data %>%
  ...
  scale_y_continuous(breaks = seq(140, 200, 5)) +
  scale_fill_manual(values = c("Odd" = "#C6011F", "Even" = "#000000")) +
  ...
  labs(title = "Cincinnati Reds Franchise History\nMost Home Runs in a 5 Season Span",
