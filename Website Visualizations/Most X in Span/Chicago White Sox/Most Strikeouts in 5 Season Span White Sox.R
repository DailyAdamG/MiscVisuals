...
setwd("C:/Users/daily/Desktop/Repositories/MiscVisuals/Website Visualizations/Most X in Span/Chicago White Sox")
...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, TeamsFranchises.franchID, SUM(Pitching.SO) AS SO
                    ...
                    WHERE TeamsFranchises.franchName = "Chicago White Sox"
...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(SO_Total = rollapply(SO, width = 5, FUN = sum, align = "right",partial = TRUE)) %>%
...
top_data %>%
  ...
  coord_flip(ylim = c(700, 1200)) +
  scale_y_continuous(breaks = seq(700, 1200, 50)) +
  scale_fill_manual(values = c("Odd" = "#27251F", "Even" = "#C4CED4")) +
  ...
  labs(title = "Chicago White Sox Franchise History\nMost Strikeouts in a 5 Season Span",
