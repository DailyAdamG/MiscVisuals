...
GROUP_CONCAT(TeamsFranchises.franchName) AS Teams, Batting.yearID, TeamsFranchises.franchID, SUM(Batting.SB) AS SB
                    FROM People
                    INNER JOIN Batting
                    ...
                    HAVING SUM(Batting.SB) >= 100)
                    SELECT nameFirst, nameLast, GROUP_CONCAT(DISTINCT Teams) AS Teams, GROUP_CONCAT(DISTINCT yearID) AS Years, COUNT(1) AS Seasons, COUNT(DISTINCT Teams) AS Franchises
                    FROM sub1
                    GROUP BY playerID
...
fill_color <- c("Arizona Diamondbacks" = "#5F259F",
                ...
                "Kansas City Cowboys,Philadelphia Phillies" = "#E81828",
                "Boston Reds" = "seashell",
...
top_data %>%
  ...
  labs(title = "Most Seasons with 100 or More Stolen Bases",
       x = "",
       y = "100+ SB Seasons")