...
TeamsFranchises.franchName AS Teams, TeamsFranchises.franchID, SUM(Batting.HR) AS HR
                    ...
                    HAVING (CASE WHEN birthMonth <= 6 THEN Batting.yearID - birthYear ELSE Batting.yearID - birthYear - 1 END < 25))
              SELECT playerID,
              nameFirst,
              nameLast,
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, HR_Total), y = HR_Total, fill = Teams)) +
  ...
  labs(title = "Most Home Runs Before Age 25 Season",
       subtitle = "Using July 1st as Season Age Cutoff",
       x = "",
       y = "Total Home Runs Hit Before Age 25 Season")