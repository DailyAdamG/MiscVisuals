...
data <- data %>%
  ...
  group_by(playerID) %>%
  mutate(HR_Total = rollapply(HR, width = 5, FUN = sum, align = "left",partial = TRUE)) %>%
  mutate(firstYear = lead(yearID, 1)) %>%
  mutate(secondYear = lead(yearID,2)) %>%
  mutate(thirdYear = lead(yearID,3)) %>%
  mutate(fourthYear = lead(yearID,4)) %>%
  mutate(Range = fourthYear - yearID) %>%
  mutate(firstYearTeam = lead(Teams,1)) %>%
  mutate(secondYearTeam = lead(Teams,2)) %>%
  mutate(thirdYearTeam = lead(Teams,3)) %>%
  mutate(fourthYearTeam = lead(Teams,4)) %>%
  mutate(OneFranchise = ifelse(Teams == firstYearTeam & Teams == secondYearTeam & Teams == thirdYearTeam & Teams == fourthYearTeam, "Yes", "No")) %>%
  ungroup()
...
top_data %>%
  ...
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = HR_Total), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(150, 220)) +
  scale_y_continuous(breaks = seq(150, 220, 10)) +
  scale_fill_manual(values = fill_color) +
  theme(plot.title = element_text(size = 16, face = "bold"),
        plot.subtitle = element_text(size = 12),
        axis.title.x = element_text(size = 12, face = "bold"),
        ...
        axis.text.y = element_text(size = 10, face ="bold"),
        legend.position = "none") +
  labs(title = "Most Home Runs in a 5 Season Span \nto Start Career",
       x = "",
       y = "Home Runs")