...
data <- data %>%
  ...
  group_by(playerid) %>%
  mutate(WAR_Total = rollapply(WAR, width = 5, FUN = sum, align = "left",partial = TRUE)) %>%
  mutate(firstYear = lead(Season, 1)) %>%
  mutate(secondYear = lead(Season,2)) %>%
  mutate(thirdYear = lead(Season,3)) %>%
  mutate(fourthYear = lead(Season,4)) %>%
  mutate(Range = fourthYear - Season) %>%
  mutate(firstYearTeam = lead(Team,1)) %>%
  mutate(secondYearTeam = lead(Team,2)) %>%
  mutate(thirdYearTeam = lead(Team,3)) %>%
  mutate(fourthYearTeam = lead(Team,4)) %>%
  mutate(OneFranchise = ifelse(Team == firstYearTeam & Team == secondYearTeam & Team == thirdYearTeam & Team == fourthYearTeam, "Yes", "No")) %>%
  ungroup()
