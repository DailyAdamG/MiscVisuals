...
TeamsFranchises.franchName AS Team, TeamsFranchises.franchID, SUM(Batting.H) AS H,
MIN(Batting.yearID) AS MIN, MAX(Batting.yearID) AS MAX, COUNT(DISTINCT TeamsFranchises.franchID) AS Franchises
                    FROM People
                    INNER JOIN Batting
...
top_data %>%
  ggplot(aes(x = reorder(chart_label, H), y = H, fill = Team)) +
  geom_bar(stat = "identity", color = "black") +
  geom_text(aes(label = H), fontface = "bold", hjust = -0.3) +
  coord_flip(ylim = c(3000, 3700)) +
  scale_y_continuous(breaks = seq(3000, 3700, 50)) +
  ...
  labs(title = "Most Hits by a Player in MLB History\nWho Only Played for One Franchise",
       x = "",
       y = "Hits")